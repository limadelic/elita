defmodule El.Puppet.Collect do
  ...
  defp go(state, quiet, elapsed) do
    gap = need(state, quiet)
    decide(%{state | gap: gap}, quiet, elapsed)
  end

  defp need(%{gap: true}, _quiet), do: true
  defp need(_state, quiet), do: quiet >= 2000
  ...
  defp proceed(false, state, _quiet), do: loop(state)

  defp loop(state) do
    listen(state, wait(state))
  end

  defp listen(state, ms) do
    receive do
  ...
  defp digest(state, data) do
    ...
    collect(grow(state, data))
  end

  defp wait(state) do
    now = monotonic_time(:millisecond)
    quiet = now - state.last
    min(4000 - quiet, 60_000 - (now - state.start)) |> max(100)
  end
end
