defmodule Lite do
  ...
  defp url,
    do: "#{get_env("ANTHROPIC_BASE_URL", "https://api.anthropic.com")}/v1/messages"

  defp model, do: "claude-haiku-4-5"

  defp headers,
    do: [{"x-api-key", token()}, {"anthropic-version", "2023-06-01"}]
  ...
  defp token,
    do: ["ANTHROPIC_AUTH_TOKEN", "ANTHROPIC_API_KEY"] |> find_value(&get_env/1)

  defp resp({:ok, %{status: 200, body: %{"content" => content}}}), do: content

  defp resp({:ok, %{status: code, body: body}}),
    do: {:error, "HTTP #{code}: #{inspect(body)}"}
