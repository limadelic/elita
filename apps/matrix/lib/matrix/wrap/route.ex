defmodule Matrix.Wrap.Route do
  ...
  defp remote([name, message], agent) do
    async_nolink(Matrix.Tasks, fn -> deliver(name, message, agent) end)
    {:handled}
  end
  ...
  defp implicit([word, rest], agent) do
    async_nolink(Matrix.Tasks, fn -> deliver(word, rest, agent) end)
    {:handled}
  end
  ...
  defp sender([name, message], agent) do
    async_nolink(Matrix.Tasks, fn -> tell(name, message, agent) end)
    {:handled}
  end
