defmodule Matrix.Trace.Log do
  ...
  def chunk(data), do: jot(get_env(:matrix, :trace_log), trace(data))

  def header({rows, cols}, source) do
    jot(get_env(:matrix, :trace_log), caption(rows, cols, source))
  end

  def event(msg) do
    jot(get_env(:matrix, :trace_log), stamp(msg))
  end

  def event(msg, reason) do
    jot(get_env(:matrix, :trace_log), remark(msg, reason))
  end
